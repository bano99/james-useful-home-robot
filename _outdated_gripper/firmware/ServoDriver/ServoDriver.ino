...
// the MAC address of the device you want to ctrl (Remote Control MAC for status feedback)
...
typedef struct GripperCommand {
  int command;  // CMD_SET_POS, CMD_SET_MIDDLE, CMD_SET_TORQUE
  int id;       // Servo ID
...
GripperCommand incomingCmd;
GripperStatus outgoingStatus;
...
void setup() {
  ...
  Serial.printf("Gripper MAC: %02X:%02X:%02X:%02X:%02X:%02X\n", 
