...
void OnDataRecv(const esp_now_recv_info_t *esp_now_info, const uint8_t *incomingData, int len) {
  if(DEV_ROLE == 2){
    ...
    if (xSemaphoreTake(serial1Mutex, portMAX_DELAY) == pdTRUE) {
      switch(incomingCmd.command) {
        case CMD_SET_POS:
          st.WritePosEx(incomingCmd.id, incomingCmd.pos, abs(incomingCmd.speed), 0);
          break;
        case CMD_SET_MIDDLE:
          st.CalibrationOfs(incomingCmd.id);
          break;
        case CMD_SET_TORQUE:
          st.EnableTorque(incomingCmd.id, incomingCmd.torque);
          break;
      }
      xSemaphoreGive(serial1Mutex);
    }
    ...
    Serial.println(len);
    Serial.print("CMD: ");
    Serial.println(incomingCmd.command);
    Serial.print("POS: ");
    Serial.println(incomingCmd.pos);
  }
}
