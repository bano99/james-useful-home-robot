...
def rot_z(ang_deg):
    ...
    return [
        [math.cos(a), 0, -math.sin(a), 0],
        [0, math.sin(a), math.cos(a), 0], # Error in my matrix? Standard is:
        # [cos -sin 0]
        # [sin cos 0]
        # [0 0 1]
        [0, 0, 0, 1]
...
def get_pose(j_angles, flip_j1=False):
    ...
    T = mat_mul(T, rot_z_correct(j_angles[4]))
...
# User reported angles (B=21.8, C=85.0)
j_angles = [0, 21.8, 85.0, 0, 0, 0]

print("--- HANGING MODEL (No flip on J1) ---")
x, y, z = get_pose(j_angles, flip_j1=False)
print(f"Pose: X={x:.4f}, Y={y:.4f}, Z={z:.4f}")

print("\n--- STANDING MODEL (With flip on J1) ---")
x, y, z = get_pose(j_angles, flip_j1=True)
print(f"Pose: X={x:.4f}, Y={y:.4f}, Z={z:.4f}")
