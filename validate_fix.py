...
def get_pose(j_angles, mount_rpy=[0, 0, 3.14159], axes=[-1, 1, 1, -1, 1, 1]):
    # Mount in james.urdf.xacro
    T = translate(0.15, 0, 0.05)
    T = mat_mul(T, rpy_to_mat(mount_rpy[0], mount_rpy[1], mount_rpy[2]))
    
    # Joint 1: Stand upright
...
# Angles: B-21.8, C-85.0
j_angles = [0, -21.8, -85.0, 0, 0, 0] # Calibrator uses "Absolute" angles in cmd

print("--- TESTING PROPOSED FIX (Mount 180, J1=-1, J2=1, J3=1) ---")
# If ROS J2 is -21.8 and J3 is -85.
x, y, z = get_pose([-0, -21.8, -85.0], mount_rpy=[0, 0, 3.14159], axes=[-1, 1, 1])
print(f"EE Pose: X={x:.4f}, Y={y:.4f}, Z={z:.4f}")

print("\n--- SAME WITHOUT J1 FLIP (Mount 180, J1=1, J2=1, J3=1) ---")
x, y, z = get_pose([-0, -21.8, -85.0], mount_rpy=[0, 0, 3.14159], axes=[1, 1, 1])
print(f"EE Pose: X={x:.4f}, Y={y:.4f}, Z={z:.4f}")
